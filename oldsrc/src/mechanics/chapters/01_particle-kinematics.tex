...
\begin{note}{一些非SI时间单位}{non-si-time-units}
...
\begin{note}{一些非SI长度单位}{non-si-length-units}
...
\begin{physcite}{为什么要了解「数量级」}{order-of-magnitude-estimation}
...
\begin{definition}{质点}{definition-of-particle}
    质点是指在研究物体的运动时，将物体视为一个没有大小和形状的质量点。
...
在此处我们需要注意，确定坐标系不仅需要确定原点的位置和坐标轴的方向，还需要确定坐标轴的单位长度。
例如，在笛卡尔坐标系中，我们需要确定 $ x $、$ y $、$ z $ 轴的单位长度，这通常是通过选择一个基准长度来实现的。
这样我们就可以用3个坐标值（数值）来表示质点在参考系中的位置，从而描述质点的运动。

除了坐标方法，还有一组概念结构与坐标方法等价的描述方法，即矢量方法。
而得益于现代数学的发展，矢量方法不仅拥有坐标方法的描述能力，还具有更强的表达能力。
在此基础上还发展出了许多技巧性的方法，例如配速法，速度矢量图等。

下面我们先给出「位置矢量」的定义：
\begin{definition}{位置矢量}{definition-of-position-vector}
    位置矢量是指从参考系的原点指向质点位置的矢量，简称「位矢」
    位置矢量的长度等于质点与参考系原点之间的距离，方向由原点指向质点的位置。
\end{definition}

例如在三维笛卡尔坐标系中，如果质点的位置由坐标 $(x, y, z)$ 表示，那么位置矢量 $\symbfit{r}$ 可以表示为：
\begin{equation}
    \symbfit{r} = x \symbfit{e}_x + y \symbfit{e}_y + z \symbfit{e}_z
\end{equation}
